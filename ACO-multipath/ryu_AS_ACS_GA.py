...
class ProjectController(app_manager.RyuApp):
    ...
    def install_paths(self, src, first_port, dst, last_port, ip_src, ip_dst):
        ...
        if(para not in self.paths_dict.keys()):
            ...
            self.pw_dict[para] = []
            # alg_1 = AS(self.adjacency, self.switches,dst, src, N, Max, K_paths, p, a, b, Q, Metric)
            # alg_1.Do()
            alg_2 = ACS(self.adjacency, self.switches, dst, src, N, Max, K_paths, p, a, b, p0, Q, Metric)
            alg_2.Do()
            # alg_3 = GA(self.adjacency, self.switches,dst, src, N, Max, K_paths, Pc, Pm, Ts, Metric)
            # alg_3.Do()
            for gen in alg_2.best:
                self.paths_dict[para].append(gen.path)
                self.pw_dict[para].append(gen.fitness)
            print("Result of ACS multipath:")
