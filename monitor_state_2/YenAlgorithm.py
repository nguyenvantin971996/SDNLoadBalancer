...
for src in vertices:
    for dst in vertices:
        if src!=dst:
            para = str([dst, src])
            alg = YenAlgorithm(weight_map,vertices,src,dst,4)
            ...
            for path in paths_vertices:
                ...
                for s,d in zip(path[:-1],path[1:]):
                    w+=weight_map[s][d]
                pw.append(w)
            paths_dict[para] = paths_vertices
            pw_dict[para] = pw
with open("paths.json", "w") as f1:
    json.dump(paths_dict, f1)
with open("paths_w.json", "w") as f2:
    json.dump(pw_dict, f2)