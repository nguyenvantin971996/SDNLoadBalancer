...
class ProjectController(app_manager.RyuApp):
    ...
    def install_paths(self, src, first_port, dst, last_port, ip_src, ip_dst):
        if(len(self.paths)==0):
            st = "GA_Pc_2:"
            alg = GA(self.adjacency,self.switches,src,dst, N, Max, K_paths, Pc[0], Pm, Ts, st)
            alg1 = GA(self.adjacency,self.switches,src,dst, N, Max, K_paths, Pc[1], Pm, Ts, st)
            alg2 = GA(self.adjacency,self.switches,src,dst, N, Max, K_paths, Pc[2], Pm, Ts, st)
            alg3 = GA(self.adjacency,self.switches,src,dst, N, Max, K_paths, Pc[3], Pm, Ts, st)
            alg4 = GA(self.adjacency,self.switches,src,dst, N, Max, K_paths, Pc[4], Pm, Ts, st)
            alg5 = GA(self.adjacency,self.switches,src,dst, N, Max, K_paths, Pc[5], Pm, Ts, st)
            alg1.population = copy.deepcopy(alg.population)
            alg2.population = copy.deepcopy(alg.population)
            alg3.population = copy.deepcopy(alg.population)
            alg4.population = copy.deepcopy(alg.population)
            alg5.population = copy.deepcopy(alg.population)
            alg.Do()
            alg1.Do()
            alg2.Do()
            alg3.Do()
            alg4.Do()
            alg5.Do()
            a1 = [alg.lines,alg1.lines,alg2.lines,alg3.lines,alg4.lines,alg5.lines]
            ve = draw(a1,Pc,"Pc")
            ve.Do()
            for gen in alg.best:
