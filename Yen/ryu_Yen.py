...
class ProjectController(app_manager.RyuApp):
    ...
    def install_paths(self, src, first_port, dst, last_port, ip_src, ip_dst):
        ...
        if(para not in self.paths_dict.keys()):
            ...
            # alg_1.Do()
            alg_2 = YenAlgorithm(self.switches,dst,src,K_paths,Metric)
            pathss = alg_2.compute_shortest_paths()
            for gen in pathss:
                self.paths_dict[para].append(gen.path_vertices)
                self.pw_dict[para].append(gen.dictance)
            print("Result of Yen multipath:")
